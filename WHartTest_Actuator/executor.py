...
class TestCaseConfig:
    ...
    env_config: Optional[dict] = None
    ai_execution_artifact: dict[str, Any] = field(default_factory=dict)
...
class PlaywrightExecutor:
    ...
    def _browser_launch_args(self, maximize_window: bool = False) -> list[str]:
    ...
    async def _maximize_browser_window(self, page: Page) -> None:
        """尽可能将浏览器窗口切到全屏/最大化状态。

        Playwright 的启动参数在持久化 profile / Linux 窗口管理器下不总是生效，
        这里再用 CDP 强制调整窗口，作为人工采集的最终兜底。
        """
        if self.headless or self.browser_type != 'chromium' or not self._context:
            return
        try:
            ...
            try:
                screen_size = await page.evaluate(
                    """() => ({
                        width: window.screen && (window.screen.availWidth || window.screen.width) || 1920,
                        height: window.screen && (window.screen.availHeight || window.screen.height) || 1080,
                    })"""
                )
            except Exception as exc:
                logger.debug(f'读取人工采集浏览器屏幕尺寸失败，改用默认尺寸: {exc}')
            width = int(screen_size.get('width') or 1920)
            height = int(screen_size.get('height') or 1080)
            ...
            if not window_id:
                logger.warning('无法获取浏览器窗口 ID，跳过最大化')
                return
            ...
            logger.info(f'人工采集浏览器窗口已请求尺寸 {width}x{height}')

            try:
                bounds_info = await cdp.send('Browser.getWindowBounds', {'windowId': window_id})
                logger.info(f"人工采集浏览器窗口实际边界: {bounds_info}")
            except Exception as exc:
                logger.debug(f'读取人工采集浏览器窗口边界失败: {exc}')
            ...
            try:
                ...
                logger.info(f'人工采集浏览器窗口已请求内容区尺寸 {width}x{height}')
            except Exception as exc:
                logger.debug(f'人工采集浏览器窗口内容区尺寸调整失败: {exc}')

            try:
                await page.set_viewport_size({'width': width, 'height': height})
                logger.info(f'人工采集页面 viewport 已同步为 {width}x{height}')
            except Exception as exc:
                logger.debug(f'人工采集页面 viewport 调整失败: {exc}')
        except Exception as exc:
            logger.warning(f'人工采集浏览器窗口最大化失败: {exc}')
    ...
    async def init_browser(
        ...
        launch_args = self._browser_launch_args(maximize_window=maximize_window)
        ...
        if maximize_window:
            await self._maximize_browser_window(self._page)
        logger.info(
    ...
    async def browser_session(
        ...
        """浏览器会话上下文管理器"""
        await self.init_browser(ignore_https_errors=ignore_https_errors, maximize_window=maximize_window)
        try:
            yield self._page
        finally:
            await self.close()
    ...
    @asynccontextmanager
    async def browser_session_with_trace(
        self,
    ...
    def _extract_business_keywords(self, args: dict) -> list[str]:
        ...
        generated_case = self._structured_generated_case(args)
        test_plan = self._structured_test_plan(args)
        stage_contract = build_execution_stage_contract(args)
        keywords: list[str] = []
        ...
        for action in stage_contract.flow_actions:
            add(action.get('target'))
            add(action.get('value'))
            add(action.get('expected'))
        for pattern in [
            r'[“"\'【]([^”"\'】]{2,30})[”"\'】]',
            ...
            for match in re.finditer(pattern, self._structured_requirement_corpus(args), re.IGNORECASE):
                add(match.group(1))
    ...
    def _looks_like_authentication_control(self, element: dict[str, Any]) -> bool:
    ...
    def _target_allows_authentication_controls(self, target: Optional[dict[str, Any]]) -> bool:
        if not isinstance(target, dict):
            return False
        stage = str(target.get('stage') or '').strip().lower()
        return stage == 'explicit_authentication' or stage.startswith('authentication_')
    ...
    def _build_exploration_targets(
        self,
        args: dict,
        consumed_route_labels: Optional[list[str]] = None,
    ) -> list[dict[str, Any]]:
        """按测试计划意图拆成阶段化探索目标，避免一次性按最高分乱点。"""
        stage_contract = build_execution_stage_contract(args)
        requirement_document = self._structured_requirement_document(args)
        target_module = self._clean_visible_text(
            requirement_document.get('target_module') or args.get('target_module') or ''
        )
        ...
        consumed_labels = [
            self._clean_visible_text(label)
            for label in (consumed_route_labels or [])
            if self._clean_visible_text(label)
        ]
        while explicit_route and consumed_labels:
            first_label = self._clean_visible_text(explicit_route[0].get('label'))
            if first_label != consumed_labels[0]:
                break
            explicit_route = explicit_route[1:]
            consumed_labels = consumed_labels[1:]
        completed_labels = [
            *stage_contract.authentication_entry_labels,
            *(item.get('label') for item in explicit_route if isinstance(item, dict)),
            *(consumed_route_labels or []),
        ]
        feedback_targets = self._coverage_feedback_exploration_targets(
            args,
            form_coverage_keywords,
            self._extract_list_filter_constraints(args),
        ...
        if explicit_route:
            ...
            for action_index, action in enumerate(stage_contract.flow_actions, start=1):
                operation = str(action.get('operation') or '').strip().lower()
                if operation not in {'click', 'choose'}:
                    continue
                label = self._clean_visible_text(action.get('target') or action.get('value') or action.get('expected'))
                if not label:
                    continue
                if label in (stage_contract.authentication_entry_labels or ()):
                    continue
                if any(label == item.get('label') for item in explicit_route):
                    continue
                targets.append({
                    'stage': action.get('phase') or f'contract_action_{action_index}',
                    'keywords': self._route_label_clues(label),
                    'preferred_actions': [label],
                    'coverage_keywords': form_coverage_keywords,
                    'action_id': action.get('action_id') or '',
                    'operation': operation,
                })
            targets.extend(feedback_targets)
            return targets
        ...
        if target_module:
            targets.append({
                ...
                'preferred_actions': [target_module],
            })
        ...
        if not targets:
            targets.append({
                ...
                'preferred_actions': self._extract_business_keywords(args)[:6],
            })
        targets.extend(feedback_targets)
        return targets

    def _coverage_feedback_exploration_targets(
        self,
        args: dict,
        coverage_keywords: dict[str, list[str]],
        list_filter_constraints: list[dict[str, str]],
        completed_labels: Optional[list[str]] = None,
    ) -> list[dict[str, Any]]:
        """Turn backend binding gaps into additional exploration stages."""
        coverage = (
        ...
        completed_label_set = {
            self._clean_visible_text(label)
            for label in (completed_labels or [])
            if self._clean_visible_text(label)
        }
        filter_values = [
            self._clean_visible_text(item.get('value'))
            for item in list_filter_constraints
            if isinstance(item, dict) and self._clean_visible_text(item.get('value'))
        ]
        targets: list[dict[str, Any]] = []
        action_operations = {'click', 'dblclick', 'tap', 'hover', 'press'}

        def add_keyword(bucket: list[str], value: Any) -> None:
            item = self._clean_visible_text(value).strip(' "\'“”‘’[]【】()（）:：,，.。;；')
            if len(item) >= 2 and item not in bucket:
                bucket.append(item[:80])

        for index, missing in enumerate(missing_actions, start=1):
            if not isinstance(missing, dict):
                continue
            operation = str(missing.get('operation') or '').strip().lower()
            keywords: list[str] = []
            for key in ['target', 'target_name', 'name', 'phase', 'value', 'expected']:
                add_keyword(keywords, missing.get(key))
            ...
            if not keywords:
                continue
            targets.append({
                'stage': f'coverage_assert_{index}' if operation.startswith('assert_') else f'coverage_action_{index}',
                'keywords': keywords,
                'context_keywords': filter_values,
                'preferred_actions': keywords[:6],
                'coverage_keywords': coverage_keywords,
                'required_action_id': missing.get('action_id') or '',
                'required_operation': operation,
                'strict_target_match': True,
                'observe_only': operation.startswith('assert_') or operation not in action_operations,
            })
        if any(target.get('observe_only') for target in targets):
            entry_labels: list[str] = []
            for value in (coverage_keywords.get('action') or []):
                label = self._clean_visible_text(value)
                if not label or label in completed_label_set:
                    continue
                if any(label in (target.get('keywords') or []) for target in targets):
                    continue
                if label not in entry_labels:
                    entry_labels.append(label)
            if entry_labels:
                targets.insert(0, {
                    'stage': 'coverage_entry_1',
                    'keywords': self._route_label_clues(entry_labels[0]),
                    'preferred_actions': [entry_labels[0]],
                    'coverage_keywords': coverage_keywords,
    ...
    def _form_exploration_coverage_keywords(
        ...
        result = {
            ...
            'page': list(coverage_keywords.get('page') or []),
            'action': list(coverage_keywords.get('action') or []),
        }
    ...
    def _extract_requirement_exploration_route(self, requirement: Any) -> list[dict[str, Any]]:
        """Return the business route stage contract as executor-compatible dicts."""
        return [
            item.as_dict()
            for item in build_execution_stage_contract(requirement).business_route
        ]

    def _extract_explicit_authentication_entry_labels(self, args: dict) -> list[str]:
        """Return the authentication entry labels from the stage contract."""
        return list(build_execution_stage_contract(args).authentication_entry_labels)
    ...
    def _extract_list_filter_constraints(self, args: dict) -> list[dict[str, str]]:
        ...
        patterns = [
            r'([\u4e00-\u9fa5A-Za-z0-9_]{0,16}(?:编号|编码|ID|Id|id|号码|账号|名称))\s*(?:为|是|=|:|：)\s*[“"\']?([^”"\'\s，,。；;]{3,80})[”"\']?',
            r'(?:搜索|查找|查询|筛选)[^“"\']{0,24}([\u4e00-\u9fa5A-Za-z0-9_]{0,16}(?:编号|编码|ID|Id|id|号码|账号|名称))[^“"\']{0,12}[“"\']([^”"\'\s，,。；;]{3,80})[”"\']',
        ]
    ...
    def _target_route_clues(self, target: Optional[dict[str, Any]]) -> list[str]:
        ...
        for key in ['keywords', 'route_parent', 'route_child']:
            ...
            if isinstance(value, list):
                ...
                add(value)
        route_path = target.get('route_path') if isinstance(target.get('route_path'), list) else []
        if route_path:
            add(route_path[-1])
    ...
    def _route_keyword_matches_element_label(
        self,
        keyword: Any,
        label: str,
        element: dict[str, Any],
        ...
        observed = self._clean_visible_text(label).lower()
        if not normalized or not observed:
            return False
        if normalized == observed:
            return True
        if normalized not in observed and observed not in normalized:
            return False
        role = str(element.get('role') or '').strip().lower()
        if len(normalized) <= 2 and normalized != observed:
            if role == 'link' and element.get('href'):
                return False
            return role in {'menuitem', 'treeitem', 'tab', 'button'}
        return True
    ...
    def _rank_exploration_element_for_target(
        ...
        if not label or self._looks_dangerous(label, safety_policy):
            return -1
        if self._looks_like_authentication_control(element) and not self._target_allows_authentication_controls(target):
            return -1
        if any(keyword and keyword in label for keyword in target.get('blocked_actions') or []):
            return -1
        if (
            target.get('stage') == 'target_module'
            and target.get('preferred_actions')
            and label not in set(target.get('preferred_actions') or [])
            and str(element.get('role') or '').lower() not in {'link', 'menuitem', 'treeitem', 'tab'}
        ):
            return -1
        if self._is_requirement_route_stage(target):
            route_clues = [
                *(target.get('keywords') or []),
                *(target.get('route_tokens') or []),
                *(target.get('preferred_actions') or []),
            ]
            if route_clues and not any(
                self._route_keyword_matches_element_label(keyword, label, element)
                for keyword in route_clues
            ):
                return -1
        ...
        for keyword in target.get('keywords') or []:
            ...
            if normalized == label:
                exact_match = True
                score += 1200 + min(len(normalized), 60)
    ...
    async def _perform_explicit_authentication_step(
        ...
        labels = self._extract_explicit_authentication_entry_labels(args)
        if not labels:
            return False, None, '结构化需求中没有可执行的显式认证入口步骤'
        label = ''
        candidates: list[dict[str, Any]] = []
        for candidate_label in labels:
            candidate_label = self._clean_visible_text(candidate_label)
            if not candidate_label:
                continue
            target = {
                'stage': 'explicit_authentication',
                'keywords': self._route_label_clues(candidate_label),
                'preferred_actions': [candidate_label],
            }
            ranked = [
    ...
    def _target_element_is_direct_match(self, element: dict[str, Any], target: dict[str, Any]) -> bool:
        ...
        for keyword in target.get('keywords') or []:
            if self._is_requirement_route_stage(target):
                if self._route_keyword_matches_element_label(keyword, label, element):
                    return True
                continue
            if self._target_keyword_matches_observed_text(keyword, label):
                return True
        return False

    def _target_keyword_matches_observed_text(self, keyword: Any, observed_text: Any) -> bool:
        """Match requirement target text to observed UI text without command-word rules."""
        expected = self._clean_visible_text(keyword).lower()
        observed = self._clean_visible_text(observed_text).lower()
        if not expected or not observed:
            return False
        if expected == observed or expected in observed:
            return True
        if len(observed) >= 3 and re.fullmatch(r'[a-z0-9_ -]+', observed):
            expected_tokens = [token for token in re.split(r'[^a-z0-9_]+', expected) if token]
            if observed in expected_tokens:
                return True
        if len(observed) >= 3 and observed in expected and len(observed) / max(len(expected), 1) >= 0.55:
            return True
        if len(observed) >= 2 and observed in expected and len(expected) <= 12:
            return True

        min_overlap = max(3, min(5, int(len(expected) * 0.6)))
        if min(len(expected), len(observed)) < min_overlap:
            return False
        previous = [0] * (len(observed) + 1)
        best = 0
        for left_char in expected:
            current = [0] * (len(observed) + 1)
            for index, right_char in enumerate(observed, start=1):
    ...
    def _target_state_contains_keyword(self, state: dict[str, Any], target: dict[str, Any]) -> bool:
        ...
        for keyword in target.get('keywords') or []:
            if self._target_keyword_matches_observed_text(keyword, text):
                return True
            for element in elements[:260]:
                if isinstance(element, dict) and self._target_keyword_matches_observed_text(
                    keyword,
    ...
    def _target_state_has_compatible_operation_evidence(
        self,
        state: dict[str, Any],
        target: dict[str, Any],
    ) -> bool:
        operation = str(target.get('required_operation') or target.get('operation') or '').strip().lower()
        if operation not in {'fill', 'type', 'select', 'select_option', 'choose'}:
            return False
        keywords = [
        ...
        if not keywords:
            return False
        for element in self._exploration_elements(state):
            if not isinstance(element, dict):
                continue
            actions = set(element.get('actions') or [])
            compatible = (
                operation in {'fill', 'type'} and 'fill' in actions
            ) or (
                operation in {'select', 'select_option', 'choose'}
                and ('select_option' in actions or self._looks_like_select_control(element))
            )
            if not compatible:
                continue
            labels = [
                self._clean_visible_text(element.get(key))
                for key in ['name', 'label', 'placeholder', 'form_label', 'accessible_name']
                if self._clean_visible_text(element.get(key))
            ]
            for keyword in keywords:
                ...
                for label in labels:
                    ...
                    if self._target_keyword_matches_observed_text(keyword, label):
                        return True
                    if len(lowered_label) >= 2 and lowered_label in lowered_keyword:
                        return True
        return False

    def _target_state_satisfies_observe_only(self, state: dict[str, Any], target: dict[str, Any]) -> bool:
        return (
            self._target_state_contains_keyword(state, target)
            or self._target_state_has_compatible_operation_evidence(state, target)
        )
    ...
    @staticmethod
    def _is_strict_coverage_stage(target: dict[str, Any]) -> bool:
        stage = str(target.get('stage') or '')
        return bool(target.get('strict_target_match')) or stage.startswith('coverage_action_') or stage.startswith('coverage_assert_')
    ...
    def _route_candidate_is_relevant(
        ...
        for relevant in relevant_targets:
            ...
            for keyword in keywords:
                normalized = self._clean_visible_text(keyword).lower()
                if normalized and self._route_keyword_matches_element_label(normalized, label, element):
                    return True
        return False
    ...
    def _route_candidate_relaxed_match(
        ...
        if any(
            self._route_keyword_matches_element_label(clue, label, element)
            for clue in route_clues
            if self._clean_visible_text(clue)
        ):
            return True
        ...
        if role in {'menuitem', 'treeitem', 'tab', 'link'}:
            ...
            if element.get('context_type') == 'frame' and frame_match:
                return True
        if element.get('href') and frame_match:
            return True
        return False
    ...
    def _route_target_completed(
        ...
        if clicked_next_match:
            return True
        if isinstance(next_target, dict) and self._is_requirement_route_stage(next_target):
            return self._target_keywords_in_state(after_state, next_target)
        if appended:
            return True
    ...
    def _non_form_target_completed(
        ...
        appended: bool,
        direct_match: bool,
        clicked_element: Optional[dict[str, Any]] = None,
        next_target: Optional[dict[str, Any]] = None,
    ) -> bool:
        if not direct_match:
            return False
        context_only_match = self._target_element_matches_only_context(clicked_element or {}, target)
    ...
    def _target_element_matches_only_context(self, element: dict[str, Any], target: dict[str, Any]) -> bool:
        label = self._exploration_label(element)
        if not label:
            return False
        context_keywords = [
            self._clean_visible_text(keyword)
            for keyword in (target.get('context_keywords') or [])
            if self._clean_visible_text(keyword)
        ]
        if not context_keywords:
            return False
        if not any(self._target_keyword_matches_observed_text(keyword, label) for keyword in context_keywords):
            return False
        primary_keywords = [
            self._clean_visible_text(keyword)
            for keyword in (target.get('keywords') or [])
            if self._clean_visible_text(keyword)
            and self._clean_visible_text(keyword) not in context_keywords
        ]
        return not any(
            self._target_keyword_has_non_context_evidence(label, keyword, context_keywords)
            for keyword in primary_keywords
        )

    def _target_keyword_has_non_context_evidence(
        self,
        label: str,
        keyword: str,
        context_keywords: list[str],
    ) -> bool:
        normalized_label = self._clean_visible_text(label).lower()
        normalized_keyword = self._clean_visible_text(keyword).lower()
        if not normalized_label or not normalized_keyword:
            return False
        residual = normalized_keyword
        for context_keyword in context_keywords:
            context = self._clean_visible_text(context_keyword).lower()
            if context:
                residual = residual.replace(context, ' ')
        residual = self._clean_visible_text(residual).lower()
        if not residual:
            return False
        if re.search(r'[a-z0-9]', residual):
            residual_tokens = [token for token in re.split(r'[^a-z0-9_]+', residual) if token]
            return any(len(token) >= 2 and token in normalized_label for token in residual_tokens)
        return any(char in normalized_label for char in residual if not char.isspace())
    ...
    def _rank_form_entry_probe(
        ...
        is_button_like = (
            role in {'button', 'menuitem', 'treeitem', 'tab', 'link'}
            or tag == 'button'
        ...
        if element.get('in_dialog') or element.get('dialog_name') or element.get('in_form'):
            return -1
        if role in {'menuitem', 'treeitem', 'tab', 'link'}:
            if not self._target_element_matches_target(element, target):
                return -1
        box = element.get('bounding_box') if isinstance(element.get('bounding_box'), dict) else {}
    ...
    def _list_filter_field_match_score(self, field: str, element: dict[str, Any]) -> int:
        """Match a structured filter field to an observed fillable control."""
        expected = self._clean_visible_text(field).lower()
        if not expected or not isinstance(element, dict):
            return -1
        labels = []
        for key in ['label', 'name', 'accessible_name', 'placeholder', 'form_label', 'text']:
            label = self._clean_visible_text(element.get(key)).lower()
            if label and label not in labels:
                labels.append(label)
        visible = self._clean_visible_text(self._element_visible_text(element)).lower()
        if visible and visible not in labels:
        ...
        for observed in labels:
            if observed == expected:
                ...
                best = max(best, 95)
            elif len(observed) >= 2 and observed in expected:
                best = max(best, 90 + min(len(observed), 20))
            else:
                shared = set(expected) & set(observed)
                shorter = min(len(expected), len(observed))
                if shorter >= 3 and len(shared) / shorter >= 0.8:
                    best = max(best, 60 + len(shared))
        return best
    ...
    async def _apply_list_filter_constraints(
        ...
        for constraint in constraints:
            ...
            candidates = [
                ...
                and max(
                    self._list_filter_field_match_score(clue, element)
                    for clue in field_clues
                    if clue
                ) >= 0
            ]
            if not candidates:
            ...
            candidates.sort(key=lambda item: (
                -max(
                    self._list_filter_field_match_score(clue, item)
                    for clue in field_clues
                    if clue
                ),
            ...
            if not clicked:
                return state
            await self._wait_for_filtered_list_result(page, value)
            return await self._mcp_collect_page_state(
                ...
                priority_keywords=[
                    keyword
                    for keyword in [field, value, *(priority_keywords or [])]
                    if self._clean_visible_text(keyword)
                ],
            )
        return state

    async def _wait_for_filtered_list_result(self, page: Page, expected_value: str) -> None:
        """Wait for a filter-triggered list refresh to render the target value or a stable empty state."""
        value = self._clean_visible_text(expected_value)
        try:
            if value:
            ...
            await page.wait_for_function(
                ...
                () => {
                  ...
                  const loading = Array.from(document.querySelectorAll(
                    '.el-loading-mask, .arco-spin, .ant-spin, [class*="loading"], [class*="Loading"], [aria-busy="true"]'
                  )).some(visible);
                  if (loading) return false;
                  const bodyText = String(document.body?.innerText || document.body?.textContent || '');
                  const hasRows = document.querySelectorAll(
                    'tbody tr, .el-table__row, .arco-table-tr, .ant-table-row, [role="row"]'
                  ).length > 1;
                  const hasEmpty = /暂无数据|无数据|没有数据|No Data|No records|empty/i.test(bodyText);
    ...
    async def _auto_login_if_possible(
        ...
        try:
            ...
            if self._looks_like_login_page(check_state):
                raise RuntimeError('登录后仍停留在登录页，未到达系统页')
    ...
    async def _auto_explore_business_pages(
        ...
        stage_probe_budget = max(max_steps, len([stage for stage in required_stages if stage]) * 2)
        ...
        consumed_route_labels: list[str] = []
        if authentication_mode == 'authentication_then_business' and self._looks_like_login_page(first_state):
            ...
            if not auth_success:
                ...
                observations.append({
                    'type': 'business_exploration_stop',
                    'reason': auth_error or '显式认证阶段未能到达后续业务页面',
                    'url': page.url,
                })
                observations.append({
                    'type': 'business_exploration_coverage',
            ...
            authentication_labels = self._extract_explicit_authentication_entry_labels(args)
            if explicit_route and authentication_labels:
                first_route_label = self._clean_visible_text(explicit_route[0].get('label'))
                first_auth_label = self._clean_visible_text(authentication_labels[0])
                if first_route_label and first_route_label == first_auth_label:
                    consumed_route_labels.append(first_route_label)
        ...
        if not login_success and self._looks_like_login_page(first_state):
            ...
            return pages, state_transitions
        if consumed_route_labels:
            exploration_targets = self._build_exploration_targets(args, consumed_route_labels=consumed_route_labels)
            required_stages = [str(target.get('stage') or '') for target in exploration_targets]
        observations.append({
            'type': 'business_exploration_plan',
            'targets': exploration_targets,
        ...
        for target in exploration_targets:
            ...
            target_budget = min(8, stage_probe_budget - exploration_steps)
            probes = 0
            while probes < target_budget and len(pages) < max_pages and exploration_steps < stage_probe_budget:
                current_index = len(pages) + 1
                await self._wait_for_business_state_stable(page)
                target_priority_keywords = [
                    str(value)
                    for values in (
                ...
                if self._is_strict_coverage_stage(target):
                    target_priority_keywords.extend(
                        str(value)
                        for value in (target.get('keywords') or [])
                        if self._clean_visible_text(value)
                    )
                current_state = await self._mcp_collect_page_state(
                    ...
                    max_elements=220,
                    priority_keywords=target_priority_keywords if (
                        self._target_requires_form_coverage(target)
                        or self._is_strict_coverage_stage(target)
                    ) else None,
                )
                current_state = await self._apply_list_filter_constraints(
                    ...
                    observations,
                    task_id,
                    priority_keywords=target_priority_keywords,
                )
                if str(current_state.get('page_key') or '').startswith('page_filter_'):
                    appended_filter_state = self._append_unique_page_state(pages, seen_signatures, current_state)
                    observations.append({
                        'type': 'business_exploration_filter_capture',
                        'page_key': current_state.get('page_key'),
                        'url': current_state.get('url'),
                        'element_count': len(current_state.get('elements') or []),
                        'captured': appended_filter_state,
                    })
                ...
                observe_only = bool(target.get('observe_only'))
                if observe_only and self._target_state_satisfies_observe_only(current_state, target):
                ...
                state_signature = self._page_signature(state)
                would_append = state_signature not in seen_signatures
                target_completed = (
                    form_covered if requires_form
                    else self._target_state_satisfies_observe_only(state, target) if observe_only
                    else (
                ...
                commit_state = target_completed or not self._is_requirement_route_stage(target)
                appended = self._append_unique_page_state(pages, seen_signatures, state) if commit_state else False
                if appended:
    ...
    def _locator_context(page: Page, step: dict[str, Any]):
        ...
        frame_key = str(step.get('frame_key') or '').strip()
        if not frame_key and '::' in element_key:
            frame_key = element_key.split('::', 1)[0]
        has_frame_signal = (
            str(step.get('context_type') or '') == 'frame'
            or bool(step.get('frame_name') or step.get('frame_url') or frame_key)
        )
        ...
        for frame in page.frames:
            ...
            if frame_url and (frame.url == frame_url or frame_url in frame.url or frame.url in frame_url):
                return frame
        match = re.fullmatch(r'frame_(\d+)', frame_key)
        if match:
            index = int(match.group(1)) - 1
            if 0 <= index < len(child_frames):
                return child_frames[index]
        return page
    ...
    @staticmethod
    def _select_control_selected_texts(element: dict[str, Any]) -> set[str]:
        selected = set()
        selected_values = set()
        ...
        for option in element.get('options') or []:
            ...
            if option.get('selected') or any(value and value in option_values for value in selected_values):
                selected.update(option_values)
        return selected

    @staticmethod
    def _element_has_positive_selection_state(element: dict[str, Any]) -> bool:
        if not isinstance(element, dict):
            return False
    ...
    def _step_looks_like_positive_check(self, step: dict[str, Any], operation: str) -> bool:
        if operation == 'check':
            return True
        text = self._clean_visible_text(' '.join([
        ...
        return (
            operation in {'', 'click'}
            and any(keyword in text for keyword in ['勾选', '选中', '选择'])
            and not any(keyword in text for keyword in ['取消', '未勾选', '未选中'])
        )

    @staticmethod
    def _select_option_value_for_element(element: dict[str, Any], requested: str) -> str:
        ...
        for option in element.get('options') or []:
            ...
            if requested in option_values:
                return str(option.get('value') or option.get('label') or option.get('text') or requested)
        return requested

    def _select_control_locators(
        self,
        step: dict[str, Any],
        element: dict[str, Any],
        ...
        candidates: list[dict[str, Any]] = []
        label = str(element.get('label') or element.get('form_label') or step.get('target_name') or '').strip()
        if label:
            candidates.append({'type': 'label', 'value': label, 'confidence': 0.9})
        if isinstance(locator_hint, dict):
            candidates.append(locator_hint)
        recommended = element.get('recommended_locator')
        if isinstance(recommended, dict):
            candidates.append(recommended)
        candidates.extend(item for item in (element.get('locator_candidates') or []) if isinstance(item, dict))
        candidates = self._dedupe_locators(candidates)
        priority = {
            'test_id': 0,
            'placeholder': 1,
            'id': 2,
        ...
        candidates.sort(key=lambda item: priority.get(str(item.get('type') or ''), 99))
        return candidates

    def _step_locator_candidates(
        self,
        step: dict[str, Any],
        mapped_element: dict[str, Any] | None,
        locator_hint: dict[str, Any],
        operation: str,
    ) -> list[dict[str, Any]]:
        candidates: list[dict[str, Any]] = []
        if isinstance(locator_hint, dict):
            candidates.append(locator_hint)
        candidates.extend(item for item in (step.get('locator_candidates') or []) if isinstance(item, dict))
        if isinstance(mapped_element, dict):
            recommended = mapped_element.get('recommended_locator')
            if isinstance(recommended, dict):
                candidates.append(recommended)
            candidates.extend(item for item in (mapped_element.get('locator_candidates') or []) if isinstance(item, dict))

        target_name = self._clean_visible_text(
            step.get('target_name')
            or step.get('description')
            or (locator_hint.get('name') if isinstance(locator_hint, dict) else '')
            or (locator_hint.get('value') if isinstance(locator_hint, dict) else '')
        )
        if operation in {'click', 'check', 'uncheck', 'assert_visible'} and target_name:
            candidates.extend([
                {'type': 'role', 'value': 'link', 'name': target_name, 'confidence': 0.72},
                {'type': 'role', 'value': 'button', 'name': target_name, 'confidence': 0.72},
                {'type': 'text', 'value': target_name, 'confidence': 0.68},
            ])

        return self._dedupe_locators(candidates)
    ...
    async def _execute_ai_plan_step(
        ...
        if (
            mapped_element
            and self._step_looks_like_positive_check(step, operation)
            and self._element_has_positive_selection_state(mapped_element)
        ):
            return True, f'步骤执行成功: {operation}，目标已处于选中态', locator_hint
        ...
        if operation == 'select_option':
            if mapped_element and (
                str(mapped_element.get('tag') or '').strip().lower() == 'select'
                or 'select_option' in (mapped_element.get('actions') or [])
            ):
                if value and value in self._select_control_selected_texts(mapped_element):
                    return True, f'步骤执行成功: {operation}，当前选项已满足', locator_hint
                option_value = self._select_option_value_for_element(mapped_element, value)
                direct_errors: list[str] = []
                for candidate in self._select_control_locators(step, mapped_element, locator_hint):
                    try:
                        locator = await self._first_visible_locator(
                            self._candidate_locator(locator_context, candidate, 'select_option'),
                            timeout=2500,
                        )
                        if option_value:
                            try:
                            ...
                            await page.keyboard.press('Enter')
                        await self._wait_for_ui_interaction_ready(page)
                        return True, f'步骤执行成功: {operation}', candidate
                    except Exception as exc:
                        direct_errors.append(str(exc)[:300])
                if direct_errors and not value:
                    raise RuntimeError('; '.join(direct_errors[-2:]))
            selected, selected_locator = await self._select_custom_combobox_option(
                page,
                step,
                locator_context,
        ...
        locator = None
        locator_used = locator_hint
        locator_errors: list[dict[str, str]] = []
        for candidate_hint in self._step_locator_candidates(step, mapped_element, locator_hint, operation):
            try:
                locator = await self._first_visible_locator(
                    self._candidate_locator(locator_context, candidate_hint, operation),
                    timeout=5000 if candidate_hint == locator_hint else 2500,
                )
                locator_used = candidate_hint
                break
            except Exception as exc:
                locator_errors.append({
                    'locator': str(candidate_hint)[:240],
                    'error': str(exc)[:300],
                })
        if locator is None:
            if locator_errors:
                raise TimeoutError(f"当前页面未找到可见目标，已尝试 {len(locator_errors)} 个 locator 候选: {locator_errors[-3:]}")
            raise TimeoutError('当前页面未找到可见目标: 没有可执行 locator 候选')
        ...
        return True, f'步骤执行成功: {operation}', locator_used
        ...
        state = await page.evaluate(
            ...
            ({maxElements, priorityKeywords}) => {
              const selector = [
                ...
                '.el-table__row td .cell', '.el-table__row td',
                'tbody tr td .cell', 'tbody tr td',
                '.ant-table-row td', '.arco-table-tr td',
              ...
              const keywordScore = (text) => {
                ...
                for (const keyword of keywords) {
                  if (!keyword) continue;
                  if (normalized === keyword) score += 1800 + Math.min(keyword.length, 80);
                  else if (normalized.includes(keyword) || keyword.includes(normalized)) score += 1200 + Math.min(keyword.length, 80);
                }
                return score;
              };
              ...
              const tableContext = (el) => {
                const row = el.closest(
                  'tbody tr, .el-table__row, .ant-table-row, .arco-table-tr, [role="row"]'
                );
                if (!row) return {};
                const table = row.closest(
                  'table, .el-table, .ant-table, .arco-table, [role="table"], [role="grid"]'
                );
                const rows = table
                  ? Array.from(table.querySelectorAll('tbody tr, .el-table__row, .ant-table-row, .arco-table-tr, [role="row"]'))
                ...
                const headers = table
                  ...
                  : [];
                const cells = Array.from(row.querySelectorAll('td, th, [role="cell"], [role="gridcell"], .cell'))
                  .map((cell, cellIndex) => ({
                    index: cellIndex,
                    column_index: cellIndex,
                    column_name: headers[cellIndex] || '',
                    text: textOf(cell).slice(0, 160),
                  }))
                  .filter((cell) => cell.text);
                const currentCell = el.closest('td, th, [role="cell"], [role="gridcell"]');
                const columnIndex = currentCell
                  ? Array.from(currentCell.parentElement?.children || []).indexOf(currentCell)
                  : -1;
                const labelledBy = table ? attr(table, 'aria-labelledby') : '';
                const labelledByText = labelledBy
                  ? labelledBy.split(/\\s+/).map((id) => textOf(document.getElementById(id))).filter(Boolean).join(' ')
                  : '';
                const caption = table
                  ? (
                    textOf(table.querySelector('caption, .el-table__header-wrapper, .ant-table-title, .arco-table-title')) ||
                    attr(table, 'aria-label') ||
                    labelledByText ||
                    attr(table, 'title')
                  )
                  : '';
                return {
                  row_text: textOf(row).slice(0, 600),
                  row_index: rowIndex >= 0 ? rowIndex : undefined,
                  row_cells: cells.slice(0, 24),
                  column_index: columnIndex >= 0 ? columnIndex : undefined,
                  column_name: columnIndex >= 0 && headers[columnIndex] ? headers[columnIndex] : '',
                  column_text: currentCell ? textOf(currentCell).slice(0, 200) : '',
                  table_name: caption.slice(0, 160),
                };
              };
              const inferRole = (el) => {
              ...
              const elementPriority = (el) => {
                ...
                if (el.closest('tbody tr, .el-table__row, .ant-table-row, .arco-table-tr') && text) score += 35;
                const cursor = window.getComputedStyle(el).cursor;
                const className = String(attr(el, 'class') || '');
                if (cursor === 'pointer') score += 90;
                if (/link|click|pointer/i.test(className)) score += 80;
                return score;
              };
              const elements = Array.from(document.querySelectorAll(selector))
                ...
                .map((el, index) => {
                  ...
                  const rowCtx = tableContext(el);
                  const className = String(attr(el, 'class') || '');
                  const ariaChecked = attr(el, 'aria-checked');
                  const ariaSelected = attr(el, 'aria-selected');
                  const dataState = attr(el, 'data-state');
                  const checked = typeof el.checked === 'boolean' ? el.checked : undefined;
                  const selected = typeof el.selected === 'boolean' ? el.selected : undefined;
                  const stateText = [className, ariaChecked, ariaSelected, dataState, checked === undefined ? '' : String(checked), selected === undefined ? '' : String(selected)].filter(Boolean).join(' ');
                  ...
                  return {
                    ...
                    in_form: inForm,
                    row_text: rowCtx.row_text,
                    row_index: rowCtx.row_index,
                    row_cells: rowCtx.row_cells,
                    column_index: rowCtx.column_index,
                    column_name: rowCtx.column_name,
                    column_text: rowCtx.column_text,
                    table_name: rowCtx.table_name,
                    visible: true,
                    ...
                    state_text: stateText.slice(0, 300),
                    options,
                    current_value: tag === 'select' || tag === 'input' || tag === 'textarea' ? el.value : '',
                    selected_label: selectedOption ? textOf(selectedOption) : '',
                    selected_value: selectedOption ? attr(selectedOption, 'value') : '',
            ...
            """,
            {'maxElements': max_elements, 'priorityKeywords': priority_keywords or []},
        )
    ...
    def _build_mcp_test_plan(self, args: dict, element_map: dict) -> dict:
        ...
        def is_confirmation(element: dict[str, Any]) -> bool:
            ...
            return (
                'click' in (element.get('actions') or [])
                and self._requires_confirmation(self._element_visible_text(element), safety_policy)
            )
        ...
        requested_field_names = [
            self._clean_visible_text(action.get('target') or action.get('name') or action.get('target_name'))
            for action in stage_contract.flow_actions
            if str(action.get('operation') or '').strip().lower() in {'fill', 'select_option', 'check', 'uncheck', 'choose'}
        ]
        requested_field_names = list(dict.fromkeys([item for item in requested_field_names if item]))
        ...
        steps = [
            {
                ...
                'action': 'navigate',
                'operation': 'goto',
                'description': f"加载目标页面：{target_url}",
                'page_key': page_key,
                'element_key': '',
                'target_name': '',
                'locator_hint': {},
        ...
        for element in target_elements:
            ...
            if 'fill' in actions:
                ...
                allow_submit = bool(safety_policy.get('allow_form_submit', True))
                steps.append({
                    **base_step,
                    'action': 'click' if allow_submit else 'assert',
                    'operation': 'click' if allow_submit else 'assert_visible',
                    'description': f"{'执行确认动作' if allow_submit else '验证确认动作可见'}：{safe_name}",
                    'value': '',
                    'expected': '确认动作已触发' if allow_submit else '确认动作可见',
                    'requires_confirmation': False,
    ...
    def _build_state_transition_steps(
        ...
        label = self._clean_visible_text(transition.get('target_name') or '业务状态迁移')
        steps = [{
            'step_sort': start_sort,
            'action': 'click',
            'operation': 'click',
            'description': f'执行业务状态迁移：{label}',
    ...
    def _select_task_relevant_elements(self, args: dict, page_state: dict[str, Any], limit: int = 12) -> list[dict[str, Any]]:
        elements = self._exploration_elements(page_state)
        runtime_mode = self._clean_visible_text(args.get('execution_mode')).lower() == 'runtime_agent'
        on_login_page = self._looks_like_login_page(page_state)
        ranked = [
            (self._element_script_priority(args, element), index, element)
            for index, element in enumerate(elements)
            if (
                isinstance(element.get('recommended_locator'), dict)
                or any(
                    isinstance(item, dict) and item.get('type') and item.get('value') not in (None, '')
                    for item in (element.get('locator_candidates') or [])
                )
            )
        ]
        ranked = [item for item in ranked if item[0] > 0]
        if runtime_mode and not on_login_page:
            ranked = [item for item in ranked if not self._looks_like_authentication_control(item[2])]
  ...
  return {{ url: page.url(), title: await page.title() }};
                ...
                if self._looks_like_login_page(first_state) and authentication_mode != 'test_subject':
                    ...
                    if self._looks_like_login_page(first_state):
                        return {
                            'task_id': task_id,
                            'status': 'failed',
                            'message': '官方 Playwright MCP 自动登录未成功，未到达登录后业务页面，已停止生成仅登录页脚本',
                            'mcp_observations': observations,
                            'verification_result': {
                                'status': 'failed',
                                'provider': 'official-playwright-mcp',
    ...
    @staticmethod
    def _is_manual_capture_element_map(element_map: dict[str, Any] | None) -> bool:
        if not isinstance(element_map, dict):
            return False
        return element_map.get('capture_mode') == 'manual_capture' or element_map.get('source') == 'manual_capture'

    @staticmethod
    def _manual_capture_storage_state_path(element_map: dict[str, Any]) -> str:
        capture = element_map.get('manual_capture') if isinstance(element_map.get('manual_capture'), dict) else {}
        storage_state = capture.get('storage_state') if isinstance(capture.get('storage_state'), dict) else {}
        return str(storage_state.get('path') or '')

    @staticmethod
    def _manual_capture_stable_url(url: Any) -> str:
        parsed = urlparse(str(url or ''))
        if not parsed.scheme and not parsed.netloc:
            return str(url or '').split('#', 1)[0]
        query_parts = []
        for raw_part in (parsed.query or '').split('&'):
            if not raw_part:
                continue
            key, _, value = raw_part.partition('=')
            if not key:
                continue
            normalized_value = re.sub(r'\d{4,}', '<num>', value)
            normalized_value = re.sub(r'[0-9a-fA-F]{12,}', '<hex>', normalized_value)
            query_parts.append(f'{key}={normalized_value}')
        query = '&'.join(sorted(query_parts))
        base = f'{parsed.scheme}://{parsed.netloc}{parsed.path}'
        return f'{base}?{query}' if query else base

    @staticmethod
    def _manual_capture_stable_text(value: Any) -> str:
        text = re.sub(r'\s+', ' ', str(value or '')).strip()
        text = re.sub(r'\b\d{4,}\b', '<num>', text)
        text = re.sub(r'[0-9a-fA-F]{12,}', '<hex>', text)
        text = re.sub(r'\d{4}[-/]\d{1,2}[-/]\d{1,2}(?:\s+\d{1,2}:\d{1,2}(?::\d{1,2})?)?', '<date>', text)
        return text[:120]

    def _manual_capture_element_semantic_key(self, element: dict[str, Any]) -> str:
        locator = element.get('recommended_locator') if isinstance(element.get('recommended_locator'), dict) else {}
        pieces = [
            element.get('role') or '',
            element.get('tag') or element.get('tag_name') or '',
            element.get('name') or element.get('accessible_name') or '',
            ...
            'dialog' if element.get('in_dialog') or element.get('dialog_name') else '',
            'form' if element.get('in_form') or element.get('form_name') else '',
        ]
        actions = element.get('actions') if isinstance(element.get('actions'), list) else []
        normalized = [
            self._manual_capture_stable_text(piece)
            for piece in [*pieces, '|'.join(sorted(str(action) for action in actions))]
            if self._manual_capture_stable_text(piece)
        ]
        return '|'.join(normalized)

    def _page_state_signature(self, state: dict[str, Any]) -> str:
        semantic_elements: set[str] = set()
        for element in (state.get('elements') or []):
            if not isinstance(element, dict):
                continue
            key = self._manual_capture_element_semantic_key(element)
            if key:
                semantic_elements.add(key)

        frame_keys = []
        for frame in state.get('frames') or []:
            if not isinstance(frame, dict):
                continue
            frame_keys.append('|'.join([
                self._manual_capture_stable_url(frame.get('url') or ''),
                self._manual_capture_stable_text(frame.get('name') or frame.get('title') or ''),
            ]))

        raw = json.dumps({
            'url': self._manual_capture_stable_url(state.get('url') or ''),
            'title': self._manual_capture_stable_text(state.get('title') or ''),
            'elements': sorted(semantic_elements)[:260],
            'frames': sorted(item for item in frame_keys if item)[:40],
        }, ensure_ascii=False, sort_keys=True, default=str)
        return hashlib.sha256(raw.encode('utf-8')).hexdigest()

    @staticmethod
    def _manual_capture_element_count(state: dict[str, Any]) -> int:
        return len(state.get('elements') or []) if isinstance(state, dict) else 0

    def _merge_manual_capture_duplicate_page(
        self,
        existing: dict[str, Any],
        current: dict[str, Any],
        action_trace_count: int,
        screenshot_path: str,
    ) -> None:
        existing_meta = existing.get('manual_capture') if isinstance(existing.get('manual_capture'), dict) else {}
        current_count = self._manual_capture_element_count(current)
        existing_count = self._manual_capture_element_count(existing)
        stable_page_key = existing.get('page_key') or current.get('page_key')
        if current_count >= existing_count:
            preserved_first = existing_meta.get('first_captured_at') or existing_meta.get('captured_at')
            preserved_count = int(existing_meta.get('merged_snapshot_count') or 1)
            existing.clear()
            existing.update(current)
            existing['page_key'] = stable_page_key
            existing['manual_capture'] = {
                **(current.get('manual_capture') if isinstance(current.get('manual_capture'), dict) else {}),
                'first_captured_at': preserved_first,
                'last_captured_at': time.time(),
                'action_trace_count': action_trace_count,
                'screenshot': screenshot_path,
                'merged_snapshot_count': preserved_count + 1,
            }
            return
        ...
        existing['manual_capture'] = {
            **existing_meta,
            'last_captured_at': time.time(),
            'action_trace_count': action_trace_count,
            'merged_snapshot_count': int(existing_meta.get('merged_snapshot_count') or 1) + 1,
        }

    def _compact_manual_action_trace(self, action_trace: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for action in action_trace:
            ...
            target_key = json.dumps({
                ...
                'aria_label': self._manual_capture_stable_text(target.get('aria_label') or ''),
                'label': self._manual_capture_stable_text(target.get('label') or ''),
                'placeholder': self._manual_capture_stable_text(target.get('placeholder') or ''),
                'name': self._manual_capture_stable_text(target.get('name') or ''),
                'id': self._manual_capture_stable_text(target.get('id') or ''),
            }, ensure_ascii=False, sort_keys=True)
            if (
                compacted
                and action_type in {'input', 'change'}
                and compacted[-1].get('type') in {'input', 'change'}
                and compacted[-1].get('_target_key') == target_key
                and str(compacted[-1].get('url') or '') == str(action.get('url') or '')
            ):
                compacted[-1] = {**action, '_target_key': target_key}
                continue
            compacted.append({**action, '_target_key': target_key})
        for action in compacted:
            action.pop('_target_key', None)
        return compacted

    async def _install_manual_capture_action_recorder(self, page: Page, action_trace: list[dict[str, Any]]) -> None:
        """记录真实用户事件，不按业务文案做规则判断。"""
        def record_action(source: dict, payload: dict):
            if not isinstance(payload, dict):
                return
            frame = source.get('frame')
            page_obj = source.get('page')
            action_trace.append({
                **payload,
                'timestamp': time.time(),
                'page_url': page_obj.url if page_obj else '',
                'frame_url': frame.url if frame else '',
            })

        await page.expose_binding('__whartRecordManualAction', record_action)
        await page.add_init_script(
            """
            (() => {
              if (window.__whartManualCaptureInstalled) return;
              window.__whartManualCaptureInstalled = true;
              ...
              const describe = (el) => {
                ...
                return {
                  ...
                  text: textOf(el),
                  value_length: typeof el.value === 'string' ? el.value.length : undefined,
                  bounding_box: {
                    x: Math.round(rect.x),
                    y: Math.round(rect.y),
                    width: Math.round(rect.width),
              ...
              const emit = (type, event, extra = {}) => {
                ...
                try {
                  window.__whartRecordManualAction(payload).catch(() => {});
                } catch (_) {}
              };
    ...
    async def run_manual_element_map_capture(self, args: dict) -> dict:
        """人工操作浏览器时持续采集页面状态，生成稳定优先的元素地图。"""
        start = time.time()
        element_map_id = args.get('element_map_id')
        dispatch_id = args.get('dispatch_id') or ''
        env_config = args.get('environment_config') or {}
        base_url = args.get('base_url') or args.get('target_url') or env_config.get('base_url') or ''
        interval_seconds = max(0.5, min(float(args.get('interval_ms') or 1000) / 1000.0, 5.0))
        max_duration_seconds = max(30, min(int(args.get('max_duration_seconds') or 600), 1800))
        pages: list[dict[str, Any]] = []
        state_transitions: list[dict[str, Any]] = []
        ...
        last_signature = ''
        last_page_key = ''
        page_index_by_signature: dict[str, int] = {}
        storage_state_path = ''
        ...
        try:
            self.headless = False
            async with self.browser_session_with_trace(
                f"manual_element_map_{element_map_id}",
                ignore_https_errors=self._ignore_https_errors_for_env(env_config),
                maximize_window=True,
            ) as page:
                self._setup_page_listeners(page)
                await self._install_manual_capture_action_recorder(page, action_trace)
                await self._navigate_for_observation(page, base_url)
                observations.append({
                    'type': 'manual_capture_started',
                    'base_url': base_url,
                    'interval_seconds': interval_seconds,
                    'max_duration_seconds': max_duration_seconds,
                })
                while not self._stop_requested and time.time() - start < max_duration_seconds:
                    if page.is_closed():
                        observations.append({'type': 'manual_capture_page_closed', 'elapsed': time.time() - start})
                        break
                    page_key = f"manual_page_{len(pages) + 1}"
                    screenshot_path = f"{self.screenshot_dir}/manual_element_map_{element_map_id}_{page_key}.png"
                    try:
                        await self._wait_for_ui_interaction_ready(page)
                        state = await self._mcp_collect_page_state(
                            page,
                        ...
                        observations.append({
                            'type': 'manual_capture_snapshot_failed',
                            'error': f"{type(exc).__name__}: {str(exc)[:500]}",
                        })
                        await asyncio.sleep(interval_seconds)
                        continue

                    signature = self._page_state_signature(state)
                    existing_index = page_index_by_signature.get(signature)
                    if existing_index is not None and 0 <= existing_index < len(pages):
                        self._merge_manual_capture_duplicate_page(
                            pages[existing_index],
                            state,
                            len(action_trace),
                            screenshot_path,
                        )
                        observations.append({
                            'type': 'manual_capture_snapshot_merged',
                            'page_key': pages[existing_index].get('page_key'),
                            'url': state.get('url') or '',
                            'title': state.get('title') or '',
                            'element_count': len(state.get('elements') or []),
                        ...
                        last_page_key = pages[existing_index].get('page_key') or last_page_key
                    elif signature != last_signature:
                        state['manual_capture'] = {
                            'snapshot_index': len(pages) + 1,
                            'captured_at': time.time(),
                            'first_captured_at': time.time(),
                            'last_captured_at': time.time(),
                            'action_trace_count': len(action_trace),
                            'screenshot': screenshot_path,
                        ...
                        pages.append(state)
                        page_index_by_signature[signature] = len(pages) - 1
                        observations.append({
                            'type': 'manual_capture_snapshot',
                            'page_key': page_key,
                            'url': state.get('url') or '',
                            'title': state.get('title') or '',
                            'element_count': len(state.get('elements') or []),
                        ...
                        if last_page_key:
                            state_transitions.append({
                                'from': last_page_key,
                                'to': page_key,
                                'trigger': 'manual_user_operation_or_page_state_change',
                                'captured_at': time.time(),
                                'url': state.get('url') or '',
                                'title': state.get('title') or '',
                                'action_trace_count': len(action_trace),
                ...
                storage_state_path = await self._capture_ai_generation_storage_state(element_map_id)

            trace_path = self.get_current_trace_path()
            safety_policy = args.get('safety_policy') or {}
            action_trace = self._compact_manual_action_trace(action_trace)
            for page_state in pages:
                for element in page_state.get('elements') or []:
                    if not isinstance(element, dict):
                        continue
            ...
            element_map = {
                ...
                'manual_capture': {
                    ...
                    'max_duration_seconds': max_duration_seconds,
                    'storage_state': {
                        'status': 'captured' if storage_state_path else 'skipped',
                ...
                'mcp_protocol': {
                    ...
                    'capability_note': '人工操作浏览器，执行器仅按页面事实持续观察并采集元素地图',
                },
                'pages': pages,
                'state_transitions': state_transitions,
                'action_trace': action_trace,
                'exploration_coverage': {
                    'status': 'success' if pages else 'incomplete',
                    'source': 'manual_capture',
                    'planning_allowed': bool(pages),
                    'required_stages': [],
                    'completed_stages': [],
                    'missing_stages': [] if pages else ['manual_capture_snapshot'],
                },
                'coverage_summary': {
                    'page_count': len(pages),
                    'element_count': sum(len(item.get('elements') or []) for item in pages),
                    ...
                    'raw_snapshot_count': sum(
                        int((page_state.get('manual_capture') or {}).get('merged_snapshot_count') or 1)
                        for page_state in pages
                        if isinstance(page_state, dict)
                    ),
                    'deduped_snapshot_count': len(pages),
                    'risk_count': len(risk_items),
                    'low_confidence_count': len(low_confidence_items),
                ...
                'risk_items': risk_items,
                'low_confidence_items': low_confidence_items,
                'manual_capture_observations': observations,
            }
            element_map['locator_baseline'] = self._build_locator_baseline(element_map)
            return {
                'element_map_id': element_map_id,
                ...
                'status': 'success' if pages else 'failed',
                'message': '人工元素地图采集完成' if pages else '人工元素地图采集未获得页面快照',
                'element_map': element_map,
                'mcp_observations': observations,
                'failure_category': '' if pages else 'environment',
                'duration': time.time() - start,
            ...
            logger.exception("人工元素地图采集失败: element_map_id=%s", element_map_id)
            return {
                'element_map_id': element_map_id,
                'dispatch_id': dispatch_id,
                'status': 'failed',
    ...
    async def run_ai_generation_task(self, args: dict) -> dict:
        ...
        if self._is_manual_capture_element_map(existing_element_map):
            manual_pages = existing_element_map.get('pages') if isinstance(existing_element_map.get('pages'), list) else []
            coverage_summary = (
                existing_element_map.get('coverage_summary')
                if isinstance(existing_element_map.get('coverage_summary'), dict)
                else {}
            )
            storage_state_path = self._manual_capture_storage_state_path(existing_element_map)
            verification_result = {
                'status': 'success' if manual_pages else 'failed',
                'provider': 'manual-capture-element-map',
                'checked_pages': len(manual_pages),
                'checked_elements': int(coverage_summary.get('element_count') or 0),
                'element_map_id': args.get('element_map_id'),
                'element_map_source': 'manual_capture',
                'notes': ['已选择人工采集元素地图，跳过自动业务路径探索'],
            }
            if storage_state_path:
                verification_result['storage_state'] = {
                    'status': 'captured',
                    'path': storage_state_path,
                    'note': '人工采集阶段浏览器登录态已保存，后续 Playwright Test 真实执行可复用',
                }
            return {
                'task_id': task_id,
                'status': 'success' if manual_pages else 'failed',
                'message': (
                    '已使用人工采集元素地图，等待 LLM 基于真实轨迹生成可执行脚本'
                    if manual_pages
                    else '人工采集元素地图没有页面快照，不能用于业务 LLM 规划'
                ),
                'test_plan': {},
                'mcp_observations': [{
                    'type': 'manual_capture_element_map_selected',
                    'element_map_id': args.get('element_map_id'),
                    'page_count': len(manual_pages),
                    'action_trace_count': len(existing_element_map.get('action_trace') or []),
                    'state_transition_count': len(existing_element_map.get('state_transitions') or []),
                }],
                'element_map': existing_element_map,
                'generated_case': {
                    'name': args.get('target_module') or 'AI 生成 UI 用例',
                    'steps': [],
                },
                ...
                'verification_result': verification_result,
                'repair_history': [],
                'failure_category': '' if manual_pages else 'exploration_coverage',
                'current_repair_round': 0,
                '_local_runtime': {'storage_state_path': storage_state_path} if storage_state_path else {},
                'duration': time.time() - start,
            }
        ...
        try:
            ...
            return {
                ...
                'message': (
                    ...
                    if planning_allowed
                    else 'Playwright 原生页面探索未覆盖全部业务目标，已停止继续 LLM 规划'
                ),
                'test_plan': {},
                'mcp_observations': observations,
                'element_map': element_map,
    ...
    def _validate_sql_permission(self, sql_config: dict[str, Any], env_config: dict) -> None:
        ...
        if keyword == 'insert' or method == 'insert':
            if not env_config.get('db_c_status', False):
                raise ValueError("当前环境未启用数据库写入操作")
            return
    ...
    async def _execute_step(
        ...
        for idx, (locator_type, locator_value, locator_index) in enumerate(locators_to_try, start=1):
            ...
            locator_timeout = 5000 if idx == 1 else 2000
            if locator_index is not None:
                candidate = candidate.nth(locator_index)
                last_candidate = candidate

            try:
                if locator_type == 'text' and locator_index is None:
                    candidate = await self._first_visible_locator(candidate, timeout=locator_timeout)
                    last_candidate = candidate
                else:
                    await candidate.wait_for(state="visible", timeout=locator_timeout)
                locator = candidate
                ...
                last_locator_error = str(exc)
                logger.warning(f"步骤 {step.step_id}: 定位器 {idx} [{locator_type}={locator_value}] 尝试失败或不可见: {exc}")

        if locator is None:
            if operation in {'assert_text', 'assert_contain_text'} and step.input_value:
                try:
                    exact = operation == 'assert_text'
                    semantic_locator = target.get_by_text(step.input_value, exact=exact)
                    await self._first_visible_locator(semantic_locator, timeout=5000)
                    logger.info(
                        "步骤 %s: 配置定位器不可见，已使用文本语义断言兜底 [%s]",
                        step.step_id,
                        step.input_value,
                    )
                    return True, "断言 text 通过（文本语义定位）", None
                except Exception as exc:
                    return (
                        False,
                        "文本断言定位失败：配置定位器不可见，且页面未出现期望文本 "
                        f"{step.input_value!r}。最后定位错误: {last_locator_error or exc}",
                        None,
                    )
            locator = last_candidate
